...
(function () {
    ...
    window.PT = {
        ...
        /**
         * Register per-character settings fields that appear in the Character Settings UI.
         * Each field is rendered natively (text input, toggle switch, or dropdown).
         * Values are saved per-character and persist across sessions.
         *
         * Field types:
         *   - "text"   — free-form text input
         *   - "toggle" — on/off switch (default should be "true" or "false")
         *   - "select" — dropdown; provide static `options` or dynamic `source`
         *
         * For select fields, use either:
         *   - `options: [{label: "Display Name", value: "stored_value"}, ...]`
         *   - `source: "sd_samplers" | "sd_models" | "sd_schedulers" | "sd_styles"`
         *     (PocketTavern fetches the list from the configured image gen backend)
         *
         * @param {string} extensionId  Your extension's unique id.
         * @param {Array}  fields       Array of field definitions.
         *
         * @example
         *   PT.registerCharacterSettings('my-ext', [
         *       { key: 'artStyle', label: 'Art Style', type: 'select', source: 'sd_styles',
         *         default: 'digital painting' },
         *       { key: 'prompt',   label: 'Custom Prompt', type: 'text', default: '' },
         *       { key: 'enabled',  label: 'Enable Feature', type: 'toggle', default: 'true' }
         *   ]);
         */
        registerCharacterSettings: function (extensionId, fields) {
            if (window.PtBridge) {
                PtBridge.registerCharacterSettings(extensionId, JSON.stringify(fields || []));
            }
        },

        /**
         * Get the current per-character value for a registered setting.
         * Returns the saved per-character value, or the field's default if not set.
         *
         * @param {string} key  The setting key (as registered in registerCharacterSettings).
         * @returns {string}    The value, or empty string if key not found.
         *
         * @example
         *   var style = PT.getCharacterSetting('artStyle');
         */
        getCharacterSetting: function (key) {
            if (!window.PtBridge) return '';
            return PtBridge.getCharacterSetting(key) || '';
        },

        /**
         * Get the current character's face as a base64 PNG.
         * Uses face detection on the avatar to crop the face and hair region.
         * Returns empty string if no avatar or no face detected.
         *
         * @returns {string}  Base64 PNG of the cropped face, or ''.
         */
        getCharacterFace: function () {
            if (!window.PtBridge) return '';
            return PtBridge.getCharacterFace() || '';
        },

        /**
         * Get the current character's full avatar as a base64 PNG.
         *
         * @returns {string}  Base64 PNG of the avatar, or ''.
         */
        ...
        /**
         * Download and import a character card from a URL.
         * Returns a Promise that resolves with true on success, false on failure.
         *
         * @param {string} url       Download URL for the character PNG.
         * @param {string} filename  Filename to save as (e.g. "character.png").
         * @returns {Promise<boolean>}
         */
        importCharacter: function (url, filename) {
            return new Promise(function (resolve) {
                var cbId = '__importCb_' + (++_callbackCounter);
                _pendingCallbacks[cbId] = resolve;
                if (window.PtBridge) PtBridge.importCharacterFromUrl(url || '', filename || '', cbId);
                else resolve(false);
            });
        },

        /**
         * Import a character card from base64-encoded PNG data.
         * Returns a Promise that resolves with true on success, false on failure.
         *
         ...
         * @returns {Promise<boolean>}
         */
        importCharacterFromBase64: function (base64, filename) {
            return new Promise(function (resolve) {
                var cbId = '__importB64Cb_' + (++_callbackCounter);
                _pendingCallbacks[cbId] = resolve;
                if (window.PtBridge) PtBridge.importCharacterFromBase64(base64 || '', filename || '', cbId);
                else resolve(false);
            });
        },
        ...
        _showToast: function (type, message) {
            if (window.PtBridge) PtBridge.showToast(type || 'info', String(message));
        },

        // ── Panel registration ───────────────────────────────────────────────

        /**
         * Register a panel UI for this extension.
         * When registered, a panel button appears in the chat UI.
         * Tapping it opens a sheet showing the extension's HTML/CSS content.
         *
         * @param {object} config  { title: string, html: string, css: string }
         */
        registerPanel: function (config) {
            var extId = window.__ptCurrentExtId || 'unknown';
            if (window.PtBridge) PtBridge.registerPanel(extId, JSON.stringify(config || {}));
        }
    };
    ...
    /** Called by Kotlin when a character import completes. */
    window.__ptImportCharacterResult = function (callbackId, success) {
        var cb = _pendingCallbacks[callbackId];
        if (cb) {
            delete _pendingCallbacks[callbackId];
