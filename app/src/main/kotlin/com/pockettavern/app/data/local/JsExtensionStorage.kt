...
class JsExtensionStorage @Inject constructor(
    ...
    private val charExtSettingsFile: File get() = File(jsExtDir, "_char_ext_settings.json")
    ...
    // ── Per-character extension settings ────────────────────────────────────

    /**
     * Get all settings for a specific extension for a specific character.
     * Returns a map of key → value.
     */
    fun getCharacterExtensionSettings(characterFile: String, extensionId: String): Map<String, String> {
        val all = loadCharExtSettings()
        val charObj = all[characterFile]?.jsonObject ?: return emptyMap()
        val extObj = charObj[extensionId]?.jsonObject ?: return emptyMap()
        return extObj.entries.associate { (k, v) -> k to v.jsonPrimitive.content }
    }

    /**
     * Get a single setting value for an extension for a character.
     * Returns null if not set.
     */
    fun getCharacterExtensionSetting(characterFile: String, extensionId: String, key: String): String? {
        val all = loadCharExtSettings()
        val charObj = all[characterFile]?.jsonObject ?: return null
        val extObj = charObj[extensionId]?.jsonObject ?: return null
        return extObj[key]?.jsonPrimitive?.content
    }

    /**
     * Set a single setting value for an extension for a character.
     */
    fun setCharacterExtensionSetting(characterFile: String, extensionId: String, key: String, value: String) {
        val all = loadCharExtSettings().toMutableMap()
        val charMap = (all[characterFile]?.jsonObject?.toMutableMap() ?: mutableMapOf())
        val extMap = (charMap[extensionId]?.jsonObject?.toMutableMap() ?: mutableMapOf())

        extMap[key] = JsonPrimitive(value)
        charMap[extensionId] = JsonObject(extMap)
        all[characterFile] = JsonObject(charMap)

        saveCharExtSettings(all)
    }

    private fun loadCharExtSettings(): Map<String, JsonElement> {
        if (!charExtSettingsFile.exists()) return emptyMap()
        return try {
            json.parseToJsonElement(charExtSettingsFile.readText()).jsonObject.toMap()
        } catch (_: Exception) { emptyMap() }
    }

    private fun saveCharExtSettings(map: Map<String, JsonElement>) {
        charExtSettingsFile.writeText(JsonObject(map).toString())
    }
