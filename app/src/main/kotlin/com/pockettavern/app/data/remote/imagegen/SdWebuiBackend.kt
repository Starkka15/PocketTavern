...
import com.pockettavern.app.data.remote.dto.forge.OverrideSettings
...
class SdWebuiBackend(
    ...
    override suspend fun getStyles(): Result<List<String>> {
        return try {
            val styles = api.getPromptStyles()
            Result.Success(styles.map { it.name })
        } catch (e: Exception) {
            Result.Error(Exception("Failed to get styles: ${e.message}", e))
        }
    }
    ...
    override fun generate(params: ForgeGenerationParams): Flow<GenerationState> = flow {
        ...
        try {
            val overrides = params.modelOverride?.takeIf { it.isNotBlank() }?.let {
                OverrideSettings(sdModelCheckpoint = it)
            }
